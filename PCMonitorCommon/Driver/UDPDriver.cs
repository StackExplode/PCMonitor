...
public class UDPDriver : IComDriver
{
    ...
    public void SendData(IComClient client, byte[] data)
    {
        if (_running)
        {
            var task = _udpserver.SendAsync(data, data.Length, (client as PCMUDPClient).Client);
    ...
    private void ReceiveDataAsync(IAsyncResult ar)
    {
        IPEndPoint ip = null;
        byte[] arr;
        PCMUDPClient cl = null;
        try
        {
            arr = _udpserver.EndReceive(ar, ref ip);
            if (arr != null && arr.Length > 0)
            {
                cl = new PCMUDPClient(ip, this._udpserver);
        ...
        {
            OnTransmitError?.Invoke(cl ?? new PCMUDPClient(ip, this._udpserver), ex);
        }
        finally
        {
